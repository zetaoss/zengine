...
class Httpy {
  ...
  async delete<T = unknown>(url: string, options: HttpyOptions<T> = {}): Promise<HttpyResult<T>> {
    try {
  ...
  async put<T = unknown>(url: string, body: Record<string, unknown> = {}, options: HttpyOptions<T> = {}): Promise<HttpyResult<T>> {
    try {
