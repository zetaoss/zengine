...
import { renderFinalPrompt } from './promptRenderer'
...
describe('renderFinalPrompt', () => {
  ...
  it('should render {# name:list} as markdown bullet list', () => {
    const template = '{# 추천분류:list}'
    const result = renderFinalPrompt({
      ...defaultArgs,
      template,
      customFieldValues: { 추천분류: 'foo\n\nbar\n' },
    })

    expect(result).toBe('# 추천분류\n- foo\n- bar')
  })
