...
{#if open}
  ...
  >
    <div class="relative w-full max-w-xl rounded border border-a-neutral-200/70 bg-background p-5 text-left text-a-neutral-900 shadow-xl">
      <button
        aria-label="Close consent dialog"
        class="absolute right-4 top-4 cursor-pointer text-a-neutral-500 hover:text-a-neutral-700"
        type="button"
        on:click={closeConsent}>✕</button
      >
      <div class="flex max-h-[calc(100vh-5rem)] flex-col">
        <div class="mb-3 flex items-center gap-2 border-b border-a-neutral-200 pb-2">
          <img alt="logo" class="h-7 w-7" src="/zeta.svg" />
          <span class="text-sm font-bold tracking-wide text-a-neutral-700">zetawiki.com</span>
        </div>
        <div class="z-scrollbar flex-1 overflow-y-auto pr-1">
          ...
          <p class="mb-4 text-sm text-a-neutral-600">
          ...
          <div role="heading" aria-level="3" class="mb-3 text-sm font-bold tracking-wide text-a-neutral-700">
            Manage Consent Preferences
          </div>

          <div class="space-y-3">
            <div class="rounded border border-a-neutral-200">
              <div class="flex items-center gap-3 p-3">
                <button
                  class="flex min-w-0 flex-1 cursor-pointer items-center gap-2 text-left font-semibold text-a-neutral-800"
                  type="button"
                  aria-expanded={necessaryExpanded}
                  aria-controls="necessary-desc"
                  on:click={toggleNecessary}
                >
                  <span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-a-slate-200 text-a-neutral-600">
                    <ZIcon path={mdiChevronDown} size={14} class={`transition-transform ${necessaryExpanded ? 'rotate-180' : ''}`} />
                  </span>
                  <span>Strictly necessary cookies</span>
                </button>
                <span class="text-sm font-bold text-a-neutral-700">Always active</span>
              </div>
              {#if necessaryExpanded}
                <div id="necessary-desc" class="border-t border-a-neutral-200 px-3 pb-3 pt-2 text-sm text-a-neutral-600">
            ...
            <div class="rounded border border-a-neutral-200">
              ...
              {#if analyticsExpanded}
                <div id="analytics-desc" class="border-t border-a-neutral-200 px-3 pb-3 pt-2 text-sm text-a-neutral-600">
            ...
            <div class="rounded border border-a-neutral-200">
              ...
              {#if marketingExpanded}
                <div id="marketing-desc" class="border-t border-a-neutral-200 px-3 pb-3 pt-2 text-sm text-a-neutral-600">
        ...
        <div class="mt-4 grid grid-cols-3 gap-2 border-t border-a-neutral-200 pt-3">
          <button
            class="w-full cursor-pointer rounded bg-a-neutral-700 px-3 py-2.5 text-center text-sm font-bold text-background hover:bg-a-neutral-800"
            type="button"
