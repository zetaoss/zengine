...
<script lang="ts">
  ...
  async function fetchPrompt() {
    loading = true
    const [data, err] = await httpy.get<PromptItem>(`/api/editbot/prompts/${id}`)
    loading = false
    if (err) {
      console.error(err)
      return
  ...
  async function toggleFavorite() {
    ...
    const [data, err] = await httpy.post<{ is_favorite: boolean }>(`/api/editbot/prompts/${row.id}/favorite`)
    if (err) {
  ...
  async function save() {
    ...
    const [data, err] = await httpy.post<PromptItem>('/api/editbot/prompts', row as unknown as Record<string, unknown>)
    isSaving = false
    if (err) {
      if (err.code === 409) {
    ...
    isEditing = false
    showToast('저장 완료')
    await goto(resolve('/tool/editbot/prompts'))
  }
  ...
  async function delPrompt() {
    ...
    deleting = true
    const [, err] = await httpy.delete(`/api/editbot/prompts/${row.id}`)
    deleting = false
    if (err) {
  ...
  async function checkTitleExists() {
    ...
    const [data, err] = await httpy.get<{ exists: boolean }>('/api/editbot/prompts/exists', {
      title: row.title,
      exclude_id: row.id,
    })
...
<div class="p-5">
  <div class="mb-4">
    <RouteLinkButton to="/tool/editbot/prompts" color="ghost" size="small">
      ...
      <div class="mb-4 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex shrink-0 items-center gap-2">
          {#if isEditing}
            ...
            <ZButton color="ghost" size="small" disabled={isSaving} onclick={() => (id === 0 ? void goto(resolve('/tool/editbot/prompts')) : void fetchPrompt())}>
              취소
            </ZButton>
          {:else}
