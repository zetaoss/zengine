...
<script lang="ts">
  import { mdiArrowLeft, mdiCompare, mdiOpenInNew } from '@mdi/js'
  ...
  function getWikiTitleHref(task: DocTask) {
    const exists = task.phase === 'Completed' || task.request_type !== 'create'
    return getWikiHref(task.title, exists)
  }
</script>
...
<div class="p-5">
  ...
  {:else if row}
    <div class="mb-5 flex flex-wrap items-center gap-2">
      ...
      <ZButtonLink color="ghost" size="small" href={getWikiTitleHref(row)} rel="external noopener noreferrer" target="_blank" title="문서 보기">
