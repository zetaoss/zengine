...
<div bind:this={root} class="md:group order-2 ml-auto contents md:relative md:inline-block">
  <button
    type="button"
    class={`cursor-pointer order-2 ml-auto flex h-12 w-12 items-center justify-center hover:bg-(--a-slate-800) md:w-auto md:px-3 ${open ? 'bg-(--a-slate-800)' : ''}`}
    aria-expanded={open}
    onclick={toggle}
  >
  ...
  <div
    class={`order-3 z-40 bg-(--a-slate-800) md:absolute md:right-0 md:m-1 md:rounded md:border w-full md:w-auto ${open ? 'block' : 'hidden'}`}
    role="menu"
    tabindex="-1"
  ...
  >
    <nav class="grid w-full grid-cols-3 py-1 md:block md:w-fit md:whitespace-nowrap">
      {#each links as l, i (i)}
        ...
        <a
          class="block p-2 px-8 text-xs text-[#fff] hover:bg-(--a-slate-700) hover:no-underline"
          href={l.href}
          title={l.title}
