...
func runExtensions(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: ctl extensions <command>")
	}
	...
	case "upgrade":
		return runExtensionsUpgrade()
	case "help", "-h", "--help":
		fmt.Println("usage: ctl extensions <command>")
...
func runExtensionsList() error {
	...
	rows := make([]extensionInfo, 0, len(entries))
	...
	mwzEntries, err := readYamlExtensions(filepath.Join(rootDir, "mwz", "extensions", "extensions.yaml"))
	if err != nil {
		return err
	}
...
func runExtensionsUpgrade() error {
	rootDir, err := findRepoRoot()
	if err != nil {
		return err
	}

	entries, err := readExtensionRepos(filepath.Join(rootDir, "mwz", "extensions", "extensions.yaml"))
	if err != nil {
		return err
	}
	...
	for _, entry := range entries {
		...
		desiredVersion := stripTagPrefix(entry.Tag)
		targetStat, statErr := os.Stat(targetDir)
		if statErr != nil || !targetStat.IsDir() {
			if statErr == nil && targetStat.Mode().IsRegular() {
		...
		if stripTagPrefix(currentVersion) == desiredVersion {
		...
		if err := removeExtensionDir(targetDir); err != nil {
			return err
		}
		if err := cloneExtensionRepo(entry.Repo, entry.Tag, targetDir); err != nil {
			return err
...
func mergeExtensions(base []extensionInfo, extra []extensionInfo) []extensionInfo {
	byName := make(map[string]extensionInfo, len(base)+len(extra))
	...
	rows := make([]extensionInfo, 0, len(byName))
...
func printExtensionRows(rows []extensionInfo) error {
	...
	for _, row := range rows {
		...
		mismatch := row.Version != "" && row.Tag != "" && row.Version != row.Tag && row.Version != stripTagPrefix(row.Tag)
...
func readYamlExtensions(filePath string) ([]extensionInfo, error) {
	data, err := os.ReadFile(filePath)
	if err != nil {
		return nil, err
	}

	var rows []extensionInfo
	var current *extensionInfo
	for _, rawLine := range strings.Split(string(data), "\n") {
		...
		if strings.HasPrefix(line, "- name:") {
			if current != nil && current.Name != "" && current.Tag != "" {
		...
		case strings.HasPrefix(line, "tag:"):
			current.Tag = strings.TrimSpace(strings.TrimPrefix(line, "tag:"))
		}
	}
	if current != nil && current.Name != "" && current.Tag != "" {
		rows = append(rows, *current)
...
func readExtensionRepos(filePath string) ([]extensionRepo, error) {
	data, err := os.ReadFile(filePath)
	if err != nil {
		return nil, err
	}

	var rows []extensionRepo
	var current *extensionRepo
	for _, rawLine := range strings.Split(string(data), "\n") {
		line := strings.TrimSpace(rawLine)
		if line == "" {
			continue
		...
		switch {
		case strings.HasPrefix(line, "repo:"):
			current.Repo = strings.TrimSpace(strings.TrimPrefix(line, "repo:"))
		case strings.HasPrefix(line, "tag:"):
			current.Tag = strings.TrimSpace(strings.TrimPrefix(line, "tag:"))
		}
	}
...
func removeExtensionDir(dir string) error {
