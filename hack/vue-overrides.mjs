...
const PKG_A = resolve(ROOT, 'mwz/skins/ZetaSkin/vue/package.json')
const PKG_B = resolve(ROOT, 'vue/package.json')
...
console.log(gray('[vue-overrides]'))
...
if (!isFix) {
  ...
  console.error(red('ERROR: root pnpm.overrides is out of sync with common deps'))
  console.error('')
  ...
  if (extraneous.length > 0) {
    console.error(red(`Extraneous in overrides (not common): ${extraneous.length}`))
