...
describe('linkifyWikiMatch', () => {
  ...
  it('should convert an existing wiki link', async () => {
    ...
    const result = await linkifyWikiMatch('This is a [[TestPage]] link', '[[TestPage]]');
  ...
  it('should convert a non-existing wiki link', async () => {
    ...
    const result = await linkifyWikiMatch('This is a [[MissingPage]] link', '[[MissingPage]]');
  ...
  it('should handle multiple same links', async () => {
    vi.mocked(titleExist).mockResolvedValue(true);
    const result = await linkifyWikiMatch('[[TestPage]] is linked twice: [[TestPage]].', '[[TestPage]]');
  ...
  it('should handle multiple different links (1)', async () => {
    vi.mocked(titleExist).mockResolvedValue(true);
    const result = await linkifyWikiMatch('Links: [[TestPage1]], [[TestPage2]].', '[[TestPage1]]');
    expect(result).toBe('Links: <a href="/wiki/TestPage1" class="internal">TestPage1</a>, [[TestPage2]].');
  });

  it('should handle multiple different links (2)', async () => {
    vi.mocked(titleExist).mockResolvedValue(true);
    const result = await linkifyWikiMatch('Links: [[TestPage1]], [[TestPage2]].', '[[TestPage2]]');
    expect(result).toBe('Links: [[TestPage1]], <a href="/wiki/TestPage2" class="internal">TestPage2</a>.');
  });

  it('should handle multiple different links (3)', async () => {
    vi.mocked(titleExist).mockResolvedValue(true);
    const result = await linkifyWikiMatch('Links: <a href="/wiki/TestPage1" class="internal">TestPage1</a>, [[TestPage2]].', '[[TestPage2]]');
  ...
  it('should fallback on API failure', async () => {
    vi.mocked(titleExist).mockResolvedValue(false);
    const result = await linkifyWikiMatch('This is a [[ErrorPage]] link', '[[ErrorPage]]');
