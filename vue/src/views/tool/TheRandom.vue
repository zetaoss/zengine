<template>
  <div class="px-4 py-6 max-w-2xl mx-auto space-y-6 text-gray-800 dark:text-gray-100">
    <h2 class="text-3xl font-bold text-center dark:text-white">Random String Generator</h2>
    ...
    <div class="space-y-2">
      ...
      <div class="flex flex-wrap gap-2">
        <button v-for="btn in charsetButtons" :key="btn.key" @click="toggleCharset(btn.key)"
          ...
          :title="isCharsetReadonly() ? 'Cannot modify in this preset' : 'Click to toggle'">
          {{ btn.label }}
        </button>
      </div>
    </div>
    ...
    <div class="space-y-2 pt-4">
      <div v-for="(item, idx) in history" :key="idx"
        ...
        <button @click="copyToClipboard(item, idx)"
          class="text-sm px-2 py-0.5 rounded border border-gray-300 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-700 transition min-w-[80px] text-center">
...
// Character sets used per preset
...
// Visual class for charset buttons
...
// Toggle charset for custom preset
...
function generate() {
  ...
  if (preset.value === 'uuid') {
    ...
    if (!chars) {
      history.value.unshift('⚠️ Please select at least one character set.')
    } else {
      let result = ''
  ...
  copiedIndex.value = null // reset copy feedback
}
