...
const effectiveType = computed<AvatarType | null>(() => {
  ...
  if (props.tempType !== null) return props.tempType
  return isAvatarType(props.avatar.t) ? props.avatar.t : null
})
</script>
...
<template>
  <span class="inline-flex items-center justify-center overflow-hidden rounded-full box-border align-middle" :class="{
    ...
    <img v-else-if="effectiveType === 3" class="w-full h-full" alt="gravatar"
