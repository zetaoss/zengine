...
<div bind:this={root} class="md:group order-2 ml-auto contents md:relative md:inline-block">
  <button
    type="button"
    class="order-2 ml-auto flex h-12 cursor-pointer items-center px-3 hover:bg-a-gray-800"
    class:bg-a-gray-800={open}
    aria-expanded={open}
    on:click={toggle}
  >
    {#if wgUserId}
  ...
  <div
    class={`order-3 z-40 bg-a-gray-800 md:absolute md:right-0 md:m-1 md:rounded md:group-hover:block md:border w-full md:w-auto ${open ? 'block' : 'hidden'}`}
  >
    <nav class="grid grid-cols-3 w-full py-1 md:w-fit md:block md:whitespace-nowrap">
      {#each links as l, i (i)}
        <!-- svelte-ignore a11y_accesskey -->
        <a
          class="block p-2 px-8 text-xs text-background hover:bg-a-gray-700 hover:no-underline"
          href={l.href}
          title={l.title}
