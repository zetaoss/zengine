<div id="app" class="min-h-screen">
    <nav id="navbar" class="w-full text-sm border-b z-40 bg-white dark:bg-slate-700">
        <div class="max-w-8xl mx-auto w-full md:flex whitespace-nowrap">
            ...
            <navbar-user-menu :user-menu='<?= json_encode($userMenu) ?>'></navbar-user-menu>
            <div class="flex-1">
        ...
        <div class="flex flex-row <?= $hasBinders ? 'has-binders' : '' ?>">
                ...
                <div class="p-4 py-8 md:m-4 md:p-8 md:border md:rounded-md bg-white dark:bg-neutral-900">
                    ...
                    <div class="flow-root mt-2 text-sm">
                        <div class="float-left">
                            <?php if ($is_article && $action == 'view') { ?>
                                <page-meta historyhref="<?= $pageMenu['history']['href'] ?>"></page-meta>
                            <?php } ?>
                        </div>
                        <div class="flex float-right">
                            <?php foreach ($pageBtns as $k => $v) { ?>
                                <?php if ($k != $action) { ?>
                                    <?php if ($k == 'edit') { ?>
                                        <a id="ca-edit" class="page-btn" href="<?= $v['href'] ?>"><?= $v['text'] ?></a>
                                    <?php } else { ?>
                                        <a class="page-btn" href="<?= $v['href'] ?>"><?= $v['text'] == '읽기' ? '보기' : $v['text'] ?></a>
                                    <?php } ?>
                                <?php } ?>
                            <?php } ?>
                            <page-menu>
                                <?php foreach ($pageMenu as $k => $v) { ?>
                                    <li>
                    ...
                    <?php if (isset($data_portlets['data-category-normal']['html-items'])) { ?>
                        <div class="page-cats">
                            <ul>
            ...
            <?php if (! $is_specialpage) { ?>
                <div class="hidden hb-lg-md-block flex-none w-60">
                    <toc-main :datatoc='<?= json_encode($data_toc) ?>'></toc-main>
                </div>
            <?php } ?>
        </div>
    </div>
